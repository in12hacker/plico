...
- **W0.1 更新（裁决后标注）**：B-10 状态为 **deprecated compatibility retained**——
  三个被删符号（`util::safe_range`、`TemporalRange::expanded`、`Granularity::HalfYear`）
  已按 `7148f4d` 签名与行为恢复并加 `#[deprecated]`，外部视角 compile canary 见
  `tests/public_api_compat.rs`；W-05 状态为 **Ollama 真实 usage 已闭合；embedding/CAS
  estimator 仍为显式 fallback**（D-USAGE-1 登记，归后续 telemetry 设计）。

## Architecture Deviation（一处，已被裁决追认）
...
纯新增、零生产代码影响。~~请架构组在轻量 review 中追认或剔除。~~（已追认，见
`docs/milestones/v53-w0-r1-remediation.md` §1。）

## 已知限制（非本单范围，建议下轮）

- ~~`src/temporal/mod.rs:18` 仍残留 ±7 天失实表格行~~（W0.1 已修正：中置信度表述改为
  "range used exactly as resolved"，并注明 deprecated helper 不构成自动扩展）；
  temporal 各文件头部的 "LLM-based resolver" 措辞与 INDEX 中个别既有失实仍超出边界，留下轮。
