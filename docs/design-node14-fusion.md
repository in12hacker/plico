...
| **引入 LLM 做记忆链接** | Kumiho 用 LLM 做 prospective indexing | Soul 3.0 红线 2: 内核零模型; 用结构化方法先完成基础 |
...
**Soul 3.0**: 公理 3（记忆跨越边界）— tier 是边界的编码，必须可查询
...
```
**Soul 3.0**: 公理 3 — 4 层记忆是 Soul 3.0 的核心承诺
...
**Soul 3.0**: 公理 6（结构先于语言）— JSON 参数是结构化契约
...
**Soul 3.0**: 公理 1（token 稀缺）— 精确过滤节省 token
...
```
**Soul 3.0**: 公理 8（因果先于关联）— 边的类型和权重编码因果关系
...
→ "  ↑ 1 promoted" (如果条件满足)
```
**Soul 3.0**: 公理 9（越用越好）— 多次访问 = 重要 = 提升层级
...
```
**Soul 3.0**: 公理 5（机制不是策略）— tier 是机制，所有接口都应提供
...
```
**Soul 3.0**: 公理 6（结构先于语言）— 约束声明是结构化的 Guide
...
|------|---------|-------------|---------|----------|
| 总通过率 | 63/87 (72.4%) | 79/97 (**89.7%**) | dogfood 全量测试 | +2 (F-9 覆盖) |
