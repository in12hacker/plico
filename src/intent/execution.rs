...
pub fn execute_sync(
    ...
    if let Some((actions, explanation)) = recall_learned_workflow(kernel, agent_id, text) {
        let resolved: Vec<ResolvedIntent> = actions
            .iter()
            .map(|action| ResolvedIntent {
    ...
    let is_multi = resolved.len() > 1 && best.routing_action == RoutingAction::MultiAction;
    ...
    if learn && all_ok {
        let steps: Vec<crate::memory::layered::ProcedureStep> = if is_multi {
            resolved
                .iter()
                .enumerate()
                .map(|(i, ri)| crate::memory::layered::ProcedureStep {
...
pub fn execute_async(
    ...
    let action_json = serde_json::to_string(&best.action).map_err(|e| format!("Failed to serialize action: {}", e))?;
...
fn execute_actions_sequence(kernel: &AIKernel, actions: &[ApiRequest]) -> (bool, String) {
    let mut all_ok = true;
...
fn recall_learned_workflow(kernel: &AIKernel, agent_id: &str, text: &str) -> Option<(Vec<ApiRequest>, String)> {
...
fn extract_verified_workflow(procedures: &[crate::memory::MemoryEntry]) -> Option<(Vec<ApiRequest>, String)> {
    for entry in procedures {
        if !entry.tags.iter().any(|t| t == "verified") {
            continue;
        }
        if let MemoryContent::Procedure(ref proc) = entry.content {
            let actions: Vec<ApiRequest> = proc
                .steps
                .iter()
...
mod tests {
    ...
    fn make_test_kernel() -> (std::sync::Arc<AIKernel>, tempfile::TempDir) {
    ...
    fn test_execute_sync_basic_single_action() {
        let (kernel, _root) = make_test_kernel();
    ...
    fn test_execute_sync_multi_action() {
        ...
        let result = execute_sync(
            &kernel,
            &router,
            "put test1 --tags a and put test2 --tags b",
            "MultiAgent",
            0.0,
            false,
        );
    ...
    fn test_execute_sync_with_learning() {
        let (kernel, _root) = make_test_kernel();
        let router = make_router();
        let agent_id = kernel.register_agent("LearnAgent".to_string()).unwrap();

        // With learn=true, after successful execution a procedure is stored
        let result = execute_sync(&kernel, &router, "status", &agent_id, 0.0, true);
        assert!(result.is_ok());
        // If it executed successfully, a procedural memory should be stored
        let _procs = kernel.recall_procedural(&agent_id, "default", None);
    ...
    fn test_execute_sync_learn_creates_procedural() {
        let (kernel, _root) = make_test_kernel();
        let router = make_router();
        let agent_id = kernel.register_agent("LearnProceduralAgent".to_string()).unwrap();

        // With learn=true, a successful execution should create a procedural memory
        let result = execute_sync(&kernel, &router, "status", &agent_id, 0.0, true);
        assert!(result.is_ok(), "execute_sync should succeed: {result:?}");

        let memories = kernel.recall_procedural(&agent_id, "default", None);
        assert!(
            !memories.is_empty(),
            "execution should durably store the learned procedure"
        );
    }
}
