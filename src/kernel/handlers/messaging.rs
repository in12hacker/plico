...
impl super::super::AIKernel {
    pub(crate) fn handle_messaging(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ApiRequest::SendMessage { from, to, payload } => match self.send_message(&from, &to, payload) {
                Ok(msg_id) => {
            ...
            ApiRequest::DelegateTask {
                ...
                let task =
                    self.task_store
                        .create_task(task_id, from_agent, to_agent, intent, context_cids, deadline_ms);
                let mut r = ApiResponse::ok();
            ...
            ApiRequest::QueryTaskStatus { task_id } => match self.task_store.get(&task_id) {
                Some(task) => {
                    let mut r = ApiResponse::ok();
                    r.task_result = Some(crate::api::semantic::TaskResult {
                        task_id: task.task_id,
            ...
            ApiRequest::TaskStart { task_id, agent_id } => match self.task_store.start_task(&task_id, &agent_id) {
                Ok(task) => {
                    let mut r = ApiResponse::ok();
                    r.task_result = Some(crate::api::semantic::TaskResult {
                        task_id: task.task_id,
...
mod tests {
    ...
    fn grant_send_perm(kernel: &crate::kernel::AIKernel, agent: &str) {
        kernel.permission_grant(agent, crate::api::permission::PermissionAction::SendMessage, None, None);
    }

    fn grant_read_perm(kernel: &crate::kernel::AIKernel, agent: &str) {
        kernel.permission_grant(agent, crate::api::permission::PermissionAction::Read, None, None);
    }
