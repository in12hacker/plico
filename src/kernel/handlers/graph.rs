    ...
    pub(crate) fn handle_graph(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ApiRequest::Explore {
                ...
                let dto: Vec<NeighborDto> = raw
                    .into_iter()
                    .map(|(node_id, label, node_type, edge_type, authority_score)| NeighborDto {
                        node_id,
                        label,
                        node_type,
                        edge_type,
            ...
            ApiRequest::ListEdges {
                ...
                match self.kg_list_edges(&agent_id, &tenant, node_id.as_deref()) {
                    Ok(edges) => {
                        ...
                        let off = offset.unwrap_or(0);
                        let lim = limit.unwrap_or(total);
                        let dto: Vec<crate::api::semantic::KGEdgeDto> = edges
                            .into_iter()
                            .skip(off)
                            .take(lim)
                            .map(|e| crate::api::semantic::KGEdgeDto {
                                src: e.src,
                                dst: e.dst,
            ...
            ApiRequest::EdgeHistory {
                ...
                match self.kg_edge_history(&src_id, &dst_id, edge_type, &agent_id, &tenant) {
                    Ok(edges) => {
                        let dtos: Vec<crate::api::semantic::KGEdgeDto> = edges
                            .iter()
                            .map(|e| crate::api::semantic::KGEdgeDto {
                                src: e.src.clone(),
                                dst: e.dst.clone(),
            ...
            ApiRequest::KGCausalPath {
                ...
                let dtos: Vec<crate::api::semantic::CausalPathDto> = paths
                    .into_iter()
                    .map(|p| crate::api::semantic::CausalPathDto {
                        nodes: p
                            .nodes
                            .into_iter()
                            .map(|n| KGNodeDto {
            ...
            ApiRequest::KGImpactAnalysis {
                ...
                if let Ok(Some(node)) = self.kg_get_node(&node_id, &agent_id, &tenant) {
                    ...
                    if let Err(e) = self.permissions.check_namespace_access(&ctx, &node.tenant_id) {
            ...
            ApiRequest::KGTemporalChanges {
                ...
                match self.kg_temporal_changes(from_ms, to_ms, &agent_id, &tenant) {
                    Ok(changes) => {
                        let dtos: Vec<crate::api::semantic::TemporalChangeDto> = changes
                            .into_iter()
                            .map(|c| crate::api::semantic::TemporalChangeDto {
