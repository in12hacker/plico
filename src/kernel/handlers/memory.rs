//! Memory handlers — canonical durable writes, ephemeral context, recall, and stats.
        ...
        match req {
            ...
            ApiRequest::Recall {
                ...
                let memories: Vec<String> = {
                    ...
                    let mut entries = if let Some(q) = query.as_ref().filter(|q| !q.is_empty()) {
                        all_entries
                            .into_iter()
            ...
            ApiRequest::RememberLongTerm {
                ...
                let tenant = tenant_id.unwrap_or_else(|| DEFAULT_TENANT.to_string());
                match self.remember_long_term(&agent_id, &tenant, content, tags.clone(), importance) {
                    Ok(entry_id) => {
            ...
            ApiRequest::RememberProcedural {
                ...
                let proc_steps: Vec<crate::memory::layered::ProcedureStep> = steps
                    .into_iter()
                    .enumerate()
                    ...
                    .collect();
                match self.remember_procedural(
                    &agent_id,
                    DEFAULT_TENANT,
                    ops::memory::ProceduralEntry {
                        name,
                        description,
                        steps: proc_steps,
            ...
            ApiRequest::MemoryDeleteEntry {
                ...
                let tenant = tenant_id.unwrap_or_else(|| DEFAULT_TENANT.to_string());
                match self.memory_delete(&agent_id, &tenant, &entry_id) {
                    Ok(_) => ApiResponse::ok(),
            ...
            ApiRequest::MemoryStats {
                ...
                let tier_filter = tier.as_ref().and_then(|t| match t.as_str() {
