...
impl AIKernel {
    pub(crate) fn handle_core_ops(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ...
            ApiRequest::CoreList {
                ...
                offset,
                agent_id,
            } => self.core_list(variant, filter, limit, offset, agent_id),
            ApiRequest::CoreSearch {
                ...
                limit,
                agent_id,
            } => self.core_search(query, variant, filter, limit, agent_id),
            ApiRequest::CoreCreate {
                variant,
                data,
                tags,
                agent_id,
            } => self.core_create(variant, data, tags, agent_id),
            ApiRequest::CoreUpdate {
                id,
                variant,
                data,
                agent_id,
            } => self.core_update(id, variant, data, agent_id),
            ApiRequest::CoreDelete { id, variant, agent_id } => self.core_delete(id, variant, agent_id),
            ApiRequest::CoreExec {
                action,
                params,
                agent_id,
            } => self.core_exec(action, params, agent_id),
            ...
            ApiRequest::CoreLink {
                ...
                weight,
                agent_id,
            } => self.core_link(src, dst, relation, weight, agent_id),
            ApiRequest::CoreAsk {
                query,
                context_ids,
                agent_id,
            } => self.core_ask(query, context_ids, agent_id),
    ...
    fn core_get(&self, id: String, variant: Option<String>, agent_id: String) -> ApiResponse {
        ...
        match variant {
            "object" | "cas" => self.handle_cas(ApiRequest::Read {
                cid: id,
                agent_id,
                tenant_id: None,
                agent_token: None,
            }),
            "node" | "graph" => self.handle_graph(ApiRequest::GetNode {
                node_id: id,
                agent_id,
                tenant_id: None,
            }),
            "task" => self.handle_agent(ApiRequest::AgentStatus { agent_id: id }),
            "event" => self.handle_events(ApiRequest::ListEvents {
                since: None,
                until: None,
                tags: vec![format!("id:{}", id)],
                event_type: None,
                agent_id,
                limit: Some(1),
                offset: None,
    ...
    fn core_list(
        ...
        match variant {
                            ...
                            for cid in &cids[start..end] {
                                if let Ok(obj) = self.cas.get_raw(cid) {
                                    results.push(crate::api::semantic::SearchResultDto {
                                        ...
                                        snippet: crate::util::safe_truncate(&String::from_utf8_lossy(&obj.data), 200)
                                            .to_string(),
            ...
            "node" | "graph" => self.handle_graph(ApiRequest::ListNodes {
                node_type: None,
                agent_id,
                tenant_id: None,
                limit,
            ...
            "edge" => self.handle_graph(ApiRequest::ListEdges {
                node_id: None,
                agent_id,
                tenant_id: None,
                limit,
    ...
    fn core_search(
        ...
        match variant {
            ...
            "graph" => self.handle_graph(ApiRequest::Explore {
                cid: query,
                edge_type: None,
                depth: Some(1),
                agent_id,
            ...
            "keyword" => self.handle_cas(ApiRequest::Search {
                ...
                since: None,
                until: None,
                intent_context: Some("keyword".into()),
            }),
    ...
    fn core_delete(&self, id: String, variant: Option<String>, agent_id: String) -> ApiResponse {
        let variant = variant.as_deref().unwrap_or("object");
        match variant {
            "object" | "cas" => self.handle_cas(ApiRequest::Delete {
                cid: id,
                agent_id,
                tenant_id: None,
                agent_token: None,
            }),
            "node" | "graph" => self.handle_graph(ApiRequest::RemoveNode {
                node_id: id,
                agent_id,
                tenant_id: None,
            }),
            ...
            "tool_call" => {
                ...
                if let Ok(emb) = self.embedding.embed_query(&param_str) {
                    if let Some(_candidate) = self.intelligent_skill_forge.record_and_evaluate(
                        ...
                        self.event_bus
                            .emit(crate::kernel::event_bus::KernelEvent::EventCreated {
                ...
                self.handle_tools(ApiRequest::ToolCall {
                    tool: tool_name,
                    params: tool_params,
                    agent_id,
                })
    ...
    fn core_observe(&self, variant: Option<String>, agent_id: String) -> ApiResponse {
        ...
        match variant {
            "audit" => self.handle_system(ApiRequest::QueryGrowthReport {
                agent_id,
    ...
    fn core_link(
        ...
        self.handle_graph(ApiRequest::AddEdge {
            src_id: src,
            dst_id: dst,
            edge_type,
            weight,
    ...
    fn core_ask(&self, query: String, _context_ids: Vec<String>, agent_id: String) -> ApiResponse {
        self.handle_memory(ApiRequest::Recall {
            agent_id,
            query: Some(query),
            limit: Some(5),
            tier: None,
...
mod tests {
    ...
    fn test_core_exec_unsupported_action() {
        ...
        kernel.permission_grant(
            "test_agent",
            crate::api::permission::PermissionAction::Execute,
            None,
            None,
        );
