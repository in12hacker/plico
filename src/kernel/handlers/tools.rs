...
impl super::super::AIKernel {
    pub(crate) fn handle_tools(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ...
            ApiRequest::ToolDescribe { tool, agent_id: _ } => match self.tool_registry.get(&tool) {
                Some(desc) => {
                    let mut r = ApiResponse::ok();
            ...
            ApiRequest::HookList => {
                ...
                let dtos: Vec<crate::api::semantic::HookEntryDto> = hooks
                    .into_iter()
            ...
            ApiRequest::HookRegister {
                ...
                let hook_point = match point.to_lowercase().as_str() {
                    ...
                    _ => {
                        ...
                        "Unknown hook point: {}. Use: PreToolCall, PostToolCall, PreWrite, PreDelete, PreSessionStart",
                        point
                    ))
                    }
                };
                ...
                let handler: std::sync::Arc<dyn hook::HookHandler> = match action.to_lowercase().as_str() {
                    "block" => std::sync::Arc::new(ApiBlockHook {
                        tool_pattern: tool_pattern.clone(),
                        reason: reason_str,
                    }),
                    "log" | "continue" => std::sync::Arc::new(ApiLogHook {
                        tool_pattern: tool_pattern.clone(),
                    }),
                ...
                ApiResponse::ok_with_message(format!(
                    "Hook registered: {:?} priority={} action={}",
                    hook_point, prio, action
                ))
            }
