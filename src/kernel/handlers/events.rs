...
impl super::super::AIKernel {
    pub(crate) fn handle_events(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ...
            ApiRequest::EventPoll { subscription_id } => match self.event_poll(&subscription_id) {
                Some(events) => {
                    let mut r = ApiResponse::ok();
            ...
            ApiRequest::DeltaSince {
                ...
                let result = ops::delta::handle_delta_since(since_seq, watch_cids, watch_tags, limit, &self.event_bus);
                let mut r = ApiResponse::ok();
...
mod tests {
    ...
    use crate::fs::{EventRelation, EventType};
