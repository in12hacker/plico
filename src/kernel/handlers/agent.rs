...
use crate::api::semantic::{AgentDto, ApiRequest, ApiResponse};

impl super::super::AIKernel {
    pub(crate) fn handle_agent(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ApiRequest::RegisterAgent { name } => match self.register_agent(name) {
                Ok(id) => {
            ...
            ApiRequest::AgentStatus { agent_id } => match self.agent_status(&agent_id) {
            ...
            ApiRequest::AgentSuspend { agent_id } => match self.agent_suspend(&agent_id) {
                Ok(()) => ApiResponse::ok(),
                Err(e) => ApiResponse::error(e.to_string()),
            },
            ApiRequest::AgentResume { agent_id } => match self.agent_resume(&agent_id) {
                Ok(()) => ApiResponse::ok(),
                Err(e) => ApiResponse::error(e.to_string()),
            },
            ApiRequest::AgentTerminate { agent_id } => match self.agent_terminate(&agent_id) {
                Ok(()) => ApiResponse::ok(),
                Err(e) => ApiResponse::error(e.to_string()),
            },
            ApiRequest::AgentComplete { agent_id } => match self.agent_complete(&agent_id) {
                Ok(()) => ApiResponse::ok(),
                Err(e) => ApiResponse::error(e.to_string()),
            },
            ApiRequest::AgentFail { agent_id, reason } => match self.agent_fail(&agent_id, &reason) {
                Ok(()) => ApiResponse::ok(),
                Err(e) => ApiResponse::error(e.to_string()),
            },
            ...
            ApiRequest::AgentCheckpoint { agent_id } => match self.checkpoint_agent(&agent_id) {
                Ok(cid) => {
                    let mut r = ApiResponse::ok();
                    r.data = Some(cid);
                    r
            ...
            ApiRequest::AgentUsage { agent_id } => match self.agent_usage(&agent_id) {
                Some(usage) => {
                    let mut r = ApiResponse::ok();
...
mod tests {
    ...
    fn test_agent_suspend_and_resume() {
        let (kernel, _tmp) = make_kernel();
        let reg = kernel.handle_api_request(ApiRequest::RegisterAgent {
            name: "suspend_agent".to_string(),
        });
        let agent_id = reg.agent_id.unwrap();
        let resp = kernel.handle_api_request(ApiRequest::AgentSuspend {
            agent_id: agent_id.clone(),
        });
    ...
    fn test_agent_complete_and_fail() {
        ...
        let reg1 = kernel.handle_api_request(ApiRequest::RegisterAgent {
            name: "complete_agent".to_string(),
        });
        ...
        let reg2 = kernel.handle_api_request(ApiRequest::RegisterAgent {
            name: "fail_agent".to_string(),
        });
    ...
    fn test_agent_set_resources() {
        let (kernel, _tmp) = make_kernel();
        let reg = kernel.handle_api_request(ApiRequest::RegisterAgent {
            name: "res_agent".to_string(),
        });
        let agent_id = reg.agent_id.clone().unwrap();
        let resp = kernel.handle_internal_api_request(ApiRequest::AgentSetResources {
            agent_id: agent_id.clone(),
    ...
    fn test_agent_usage_not_found() {
        let (kernel, _tmp) = make_kernel();
        let resp = kernel.handle_api_request(ApiRequest::AgentUsage {
            agent_id: "nonexistent".to_string(),
        });
