...
use super::super::ops;
use crate::api::semantic::{ApiRequest, ApiResponse, ContentEncoding, DeletedDto, SearchResultDto};
    ...
    pub(crate) fn handle_cas(&self, req: ApiRequest) -> ApiResponse {
        match req {
            ...
            ApiRequest::ListDeleted { agent_id } => {
                ...
                let dto: Vec<DeletedDto> = entries
                    .into_iter()
                    .map(|e| DeletedDto {
                        cid: e.cid,
            ...
            ApiRequest::Restore { cid, agent_id } => match self.restore_deleted(&cid, &agent_id) {
                Ok(()) => ApiResponse::ok(),
                Err(e) => ApiResponse::error(e.to_string()),
            },
            ...
            ApiRequest::Rollback { cid, agent_id } => match self.rollback(&cid, &agent_id) {
                Ok(new_cid) => {
                    self.maybe_persist_search_index();
                    ApiResponse::with_cid(new_cid)
                }
...
mod tests {
    ...
    fn test_search_with_tags() {
        ...
        let resp = kernel.handle_api_request(ApiRequest::Search {
            ...
            limit: Some(10),
            offset: None,
            require_tags: vec!["important".to_string()],
            exclude_tags: vec![],
            since: None,
            until: None,
            intent_context: None,
    ...
    fn test_search_with_intent_context() {
        ...
        let resp = kernel.handle_api_request(ApiRequest::Search {
            ...
            since: None,
            until: None,
            intent_context: Some("testing".to_string()),
            agent_token: None,
        });
