...
pub(in crate::kernel) fn handle(
    ...
    match name {
        ...
        "cas.search" => {
            ...
            let require_tags: Vec<String> = params
                .get("require_tags")
                .and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                .unwrap_or_default();
            ...
            let dto: Vec<serde_json::Value> = results
                .into_iter()
                .map(|r| {
                    json!({
        ...
        "cas.update" => {
            let cid = params.get("cid").and_then(|v| v.as_str()).unwrap_or("");
            let content = params.get("content").and_then(|v| v.as_str()).unwrap_or("");
            let new_tags: Option<Vec<String>> = params
                .get("new_tags")
                .and_then(|v| v.as_array())
                .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect());
...
mod tests {
    ...
    fn test_cas_search() {
        ...
        let _ = handle(
            &kernel,
            "cas.create",
            &json!({"content": "searchable content", "tags": ["searchable"]}),
            "test_agent",
        );
        let result = handle(
            &kernel,
            "cas.search",
            &json!({"query": "searchable", "limit": 5}),
            "test_agent",
        );
        assert!(result.success);
    }
    ...
    fn test_cas_update() {
        ...
        let result = handle(
            &kernel,
            "cas.update",
            &json!({"cid": cid, "content": "updated", "new_tags": ["v2"]}),
            "test_agent",
        );
        assert!(result.success);
    }
    ...
    fn test_cas_search_with_tags() {
        ...
        let _ = handle(
            &kernel,
            "cas.create",
            &json!({"content": "tagged content", "tags": ["important", "review"]}),
            "test_agent",
        );
        let result = handle(
            &kernel,
    ...
    fn test_cas_update_not_found() {
        ...
        let result = handle(
            &kernel,
            "cas.update",
            &json!({"cid": "nonexistent", "content": "new"}),
            "test_agent",
        );
        assert!(!result.success);
    }
