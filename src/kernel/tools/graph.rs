...
pub(in crate::kernel) fn handle(
    ...
    match name {
        ...
        "kg.explore" => {
            ...
            let neighbors: Vec<serde_json::Value> = raw
                .into_iter()
                .map(|(id, label, ntype, etype, auth)| {
                    json!({
        ...
        "kg.paths" => {
            ...
            let dto: Vec<Vec<serde_json::Value>> = paths
                .into_iter()
        ...
        "kg.list_edges" => {
            ...
            match kernel.kg_list_edges(agent_id, "default", node_id) {
                Ok(edges) => {
                    let dto: Vec<serde_json::Value> = edges
                        .into_iter()
                        .map(|e| {
                            json!({
...
mod tests {
    ...
    fn test_kg_update_node() {
        ...
        let result = handle(
            &kernel,
            "kg.update_node",
            &json!({"node_id": node_id, "label": "updated"}),
            "test",
        );
        assert!(result.error.is_none());
