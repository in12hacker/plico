...
pub(in crate::kernel) fn handle(
    ...
    match name {
        ...
        "agent.set_resources" => {
            ...
            let at: Option<Vec<String>> = params
                .get("allowed_tools")
...
mod tests {
    ...
    fn test_agent_register() {
        ...
        assert!(!data["agent_id"].as_str().unwrap().is_empty());
    }
    ...
    fn test_agent_fail() {
        ...
        let _result = handle(
            &kernel,
            "agent.fail",
            &json!({"agent_id": agent_id, "reason": "test failure"}),
            "test",
        );
    }
