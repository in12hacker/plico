...
pub(crate) mod tests {
    ...
    /// Test-only debug narration for data/logic flow (visible with --nocapture;
    /// carries phases, stable categories, counts, and frozen golden digests only).
    pub(crate) fn flow(detail: impl std::fmt::Display) {
        println!("[obs] {detail}");
    }
    ...
    fn execution_observation_golden_genesis_and_request_vectors() {
        ...
        flow(format!(
            "data.hash genesis view/root = {GENESIS_VIEW_SHA} / {GENESIS_ROOT_SHA} match-golden bytes-exact"
        ));
        ...
        flow(format!(
            "data.hash requests started={STARTED_REQUEST_SHA} terminal={TERMINAL_REQUEST_SHA} match-golden bytes-exact"
        ));
    }
    ...
    fn execution_observation_golden_full_chain_and_pointers() {
        ...
        flow(format!(
            "data.chain started event/segment/view/root = {STARTED_EVENT_SHA} / {STARTED_SEGMENT_SHA} / {STARTED_VIEW_SHA} / {STARTED_ROOT_SHA}"
        ));
        flow(format!(
            "data.chain terminal event/segment/view/root = {TERMINAL_EVENT_SHA} / {TERMINAL_SEGMENT_SHA} / {TERMINAL_VIEW_SHA} / {TERMINAL_ROOT_SHA}"
        ));
        flow("data.chain 8 objects validated; bindings event->segment->root, view->root, prev-root/prev-segment ok");
        ...
        for (root_sha, golden) in [
            ...
            flow(format!(
                "data.pointer root={root_sha} canonical-bytes={} roundtrip=ok",
                golden.len()
            ));
        }
    }
    ...
    fn execution_observation_hash_domain_isolation() {
        ...
        flow(format!(
            "data.isolation same-jcs-bytes started-domain={STARTED_REQUEST_SHA} terminal-domain={cross_domain} differ"
        ));
