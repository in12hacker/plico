...
use super::error::LimitCategory::{EvidenceList, EvidenceTotal};
use super::error::{ObservationStoreError, TransitionConflictCategory};
use super::hash;
use super::hash::tests::{
    golden_key, golden_started_request, golden_terminal_request, hex64, uuid, GENESIS_ROOT_SHA, ORIGIN_REQUEST_ID,
    STARTED_EVENT_SHA, STARTED_RECORDED_AT_MS, STARTED_REQUEST_SHA, STARTED_ROOT_SHA, STARTED_SEGMENT_SHA,
    TERMINAL_EVENT_SHA, TERMINAL_RECORDED_AT_MS, TERMINAL_REQUEST_SHA, TERMINAL_SEGMENT_SHA,
};
...
fn execution_observation_evidence_list_and_total_limits() {
    ...
    assert_eq!(
        ...
        Err(ObservationStoreError::limit(EvidenceList))
    );
    ...
    assert_eq!(
        ...
        Err(ObservationStoreError::limit(EvidenceTotal))
    );
    let empty_terminal = AppendTerminalRequestV1 {
        output_evidence_cids: Vec::new(),
        ..golden_terminal_request()
    };
