...
async fn test_cognitive_conflict_detection() {
    ...
    kernel.handle_api_request(ApiRequest::CoreCreate {
        ...
        data: serde_json::Value::String(
            "However, an internal memo reveals that the CEO of Plico is actually Max.".to_string(),
        ),
        tags: vec!["t2".into()],
        agent_id: "test".into(),
    });
    ...
    let conflict_found = events
        .iter()
        .any(|e| matches!(e, KernelEvent::VerificationFailed { operation, .. } if operation == "ConflictDetection"));

    assert!(
        conflict_found,
        "Cognitive conflict should have been detected and emitted as an event"
    );
}
