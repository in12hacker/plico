...
mod cognitive_loop_tests {
    ...
    fn start_session(kernel: &plico::AIKernel, agent_id: &str) -> String {
        let resp = kernel.handle_api_request(plico::api::semantic::ApiRequest::StartSession {
            agent_id: agent_id.to_string(),
            agent_token: None,
            last_seen_seq: None,
        });
        resp.session_started.map(|s| s.session_id).unwrap_or_default()
    }
    ...
    fn test_cognitive_loop_10_rounds() {
        ...
        for round in 1..=10u32 {
            ...
            let session_id = start_session(&kernel, &agent_id);
            ...
            println!(
                "  Round {:2}: {:4}ms | session={} | assembly={:?} | ok={}",
                round,
                elapsed,
                &session_id[..8],
                resp.assembly_id.as_deref().unwrap_or("none"),
                resp.ok
            );
        ...
        let stats = rt.block_on(async { cognitive_loop.stats().await });
        println!(
        ...
        let trajectory: Vec<plico::kernel::cognition::TrajectoryPoint> =
            rt.block_on(async { tracker.get_recent_trajectory(&agent_id, 100).await });
        ...
        println!(
            "IntentCache: {} entries, {} hits, {} lookups, hit_rate={:.1}%",
            cache_stats.entries,
            cache_stats.hits,
            cache_stats.total_lookups,
            if cache_stats.total_lookups > 0 {
                cache_stats.hits as f64 / cache_stats.total_lookups as f64 * 100.0
            } else {
                0.0
            }
        );
        ...
        println!(
            "\nTiming: first 3 avg={:.0}ms, last 3 avg={:.0}ms",
        ...
        println!(
            "  IntentSemanticNetwork nodes: {}, edges: {}",
            report.new_nodes, report.new_edges
        );
    ...
    fn test_failure_pattern_tracking() {
        ...
        let failure_stats = rt.block_on(async { cognitive_loop.trajectory_tracker.get_failure_stats(&agent_id).await });
        println!(
            "TrajectoryTracker: {} total failures, {} unique operations failed",
            failure_stats.total_failures,
            failure_stats.by_operation.len()
        );
    ...
    fn test_session_lifecycle_cognitive_integration() {
        ...
        let sid1 = start_session(&kernel, &agent_id);
        ...
        // Session 2: Patch
        let sid2 = start_session(&kernel, &agent_id);
        ...
        // Session 3: Verify
        let sid3 = start_session(&kernel, &agent_id);
